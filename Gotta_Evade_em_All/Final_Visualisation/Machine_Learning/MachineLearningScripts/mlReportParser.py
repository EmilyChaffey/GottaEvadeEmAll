...
class reportParser():
    ...
    def malware(self, genTraining):
        ...
        if genTraining:
            ...
            inputdir = "../../../HOST/Reports/report.json"
            json_file = open(inputdir)
            data = json.load(json_file)
            json_file.close()
            gc.collect()
            json_str = json.dumps(data)
    ...
    def __init__(self, malicious, genTraining): #gen training is used for determining whether data is for creating training data based off of the pre-analysed reports or the new report
